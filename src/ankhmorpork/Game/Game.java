...
public class Game {
	...
	{			
		...
		if((int)Game.GameBank.getBankAmount()>= Amount)
		{
			objPayToPlayer.setPlayer_amount(objPayToPlayer.getPlayer_amount()+Amount);
			Game.GameBank.setBankAmount(Game.GameBank.getBankAmount()- Amount);
			Game.SetPlayer(objPayToPlayer);
	...
	public static int GetNoOfAreasControlledByPlayer(int PlayerID)
	{
		...
		ArrayList<Integer> iAreaControlList = new ArrayList<Integer>();
		for(int i=1; i<13; i++)
		{
			int iCount1 =0;
			int iCount2 =0;
			...
			if(AreaHasDemon(i))
			{
				for(Minion objMinion : Game.lstMinions)
				{
					if(objMinion.getArea_id()==i && objMinion.getPlayer_id()==1)
						iCount1++;
					else if(objMinion.getArea_id()==i && objMinion.getPlayer_id()==2)
						iCount2++;
					else if(objMinion.getArea_id()==i && objMinion.getPlayer_id()==3)
						iCount3++;
					else if(objMinion.getArea_id()==i && objMinion.getPlayer_id()==4)
						iCount4++;
				}

				for(Building objBuilding : Game.lstBuildings)
				{
					if(objBuilding.getArea_id()==i && objBuilding.getPlayer_id()==1)
						iCount1++;
					else if(objBuilding.getArea_id()==i && objBuilding.getPlayer_id()==2)
						iCount2++;
					else if(objBuilding.getArea_id()==i && objBuilding.getPlayer_id()==3)
						iCount3++;
					else if(objBuilding.getArea_id()==i && objBuilding.getPlayer_id()==4)
						iCount4++;
				}
				...
				if(maxCount>maxCount2)
				{
					...
					{
						if(iCount1==maxCount)
							iAreaControlList.add(1);
						if(iCount2==maxCount)
							iAreaControlList.add(2);
						if(iCount3==maxCount)
							iAreaControlList.add(3);
						if(iCount4==maxCount)
							iAreaControlList.add(4);
					}
					else
						iAreaControlList.add(0);
				}
				else
					iAreaControlList.add(0);
		...
		for(Integer i : iAreaControlList)
		{
			if(i==PlayerID)
				iPlayerAreaControlledCount++;
		}

		return iPlayerAreaControlledCount;
		//		ArrayList<Integer> lstPlayerAreaCounts = new ArrayList<Integer>();
		//		lstPlayerAreaCounts.add(iPlayer1Count);
		//		lstPlayerAreaCounts.add(iPlayer2Count);
		//		lstPlayerAreaCounts.add(iPlayer3Count);
		//		lstPlayerAreaCounts.add(iPlayer4Count);
	...
	private static int GetNoOfTrollsInArea(int iAreaID) 
	{
		...
		for(Troll objTroll : lstTrolls)
		{
			if(objTroll.getArea_id()==iAreaID)
			{
				count++;
			}
		}
